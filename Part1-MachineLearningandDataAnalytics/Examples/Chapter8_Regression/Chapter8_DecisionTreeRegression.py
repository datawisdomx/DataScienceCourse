...
# Decision Tree Regression - Bond Price prediction using macroeconomic data for US
...
# Use the data science process steps given in chapter 6 to build the Decision Tree regression model
...
# Create the multiple linear regressor
estimator = DecisionTreeRegressor(max_depth=5)

# Fit the estimator to the training data to build the model
estimator.fit(X_train, y_train)

# Plot the decision tree using sklearn export_graphviz library and graphviz module
dtgraphdata = export_graphviz(estimator)
dtgraph = graphviz.Source(dtgraphdata)
dtgraph.render("/Users/nitinsinghal/Downloads/BondPriceMacro_DecisionTreeGraph")
dtgraph
...
plt.title('Decision Tree Regression - 10yr Bond Price vs Macroeconomic factors')
