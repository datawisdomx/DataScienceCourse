...
sys.path.append('/Users/nitinsinghal/DataScienceCourse/Part1-MachineLearningDataAnalytics/Examples/Chapter14_E2EProjectRoboadvisor/')
import Chapter14_RoboadvisorDataLoadWrangling as dw, Chapter14_RoboadvisorAssetBondPricePrediction as abpp
import Chapter14_RoboadvisorPortfolioReturnRiskCalculator as prrc, Chapter14_RoboadvisorAssetBondPredVsActDirectionCalculator as adc
...
print('\n Start 3yr Asset price prediction using CB Forecasts....', datetime.datetime.now())
AllAssetsPred3Yrdata = abpp.get_AssetMthlyPricePredUsingCBFrcst(AllAssetdata, CB5MFActualdata, Macro5MFActualdata)
print('\n Start 3yr Bond price prediction using CB Forecasts....', datetime.datetime.now())
AllBondPred3Yrdata = abpp.get_BondMthlyPriceYieldPredUsingCBFrcst(AllBondYieldPricedata, CB5MFActualdata, Macro5MFActualdata)
...
# Annualized Return = ((1+apr1)*(1+apr2)...*(1+aprn))^1/n - 1. apr = asset period return
...
# Average return is the average of all returns = Sum(asset period return)/n
...
print('\n  Predicted Average Return: %.2f%%'%(predportfavgreturnval*100))
...
print('\n Predicted Portfolio Risk(Variance): %.4f, Std Dev: %.4f' %(FrcstPredPortfolioVariance, FrcstPredPortfolioStdDev))
...
print('\n Start Asset price prediction using actual data....', datetime.datetime.now())
allassetactpreddata = abpp.get_AssetMthlyPricePredUsingMacroActual(AllAssetdata, Macro5MFActualdata)
print('\n Start Bond price prediction using actual data....', datetime.datetime.now())
allbondactpreddata = abpp.get_BondMthlyPriceYieldPredUsingMacroActual(AllBondYieldPricedata, Macro5MFActualdata)
...
plt.figure()
assetcrctdiryr.plot.bar(subplots=True, figsize=(10,10), legend=False, title='Stocks % total months/year with same direction Act and Pred')
plt.xlabel('index-Year. 0->2000, 19->2019')
plt.ylabel('% Total out of 12')
plt.show()
cmdcrctdiryr.plot.bar(subplots=True, figsize=(10,10), legend=False, title='Commodities % total months/year with same direction Act and Pred')
plt.xlabel('index-Year. 0->2000, 19->2019')
plt.ylabel('% Total out of 12')
plt.show()
bondcrctdiryr.plot.bar(subplots=True, figsize=(10,10), legend=False, title='Bonds % total months/year with same direction Act and Pred')
plt.xlabel('index-Year. 0->2000, 19->2019')
plt.ylabel('% Total out of 12')
plt.show()
...
print('\n Actual Portfolio Risk(Variance): %.4f, Std Dev: %.4f' %(ActPortfolioVariance, ActPortfolioStdDev))
...
print('\n Predicted Portfolio Risk(Variance): %.4f, Std Dev: %.4f' %(PredPortfolioVariance, PredPortfolioStdDev))
