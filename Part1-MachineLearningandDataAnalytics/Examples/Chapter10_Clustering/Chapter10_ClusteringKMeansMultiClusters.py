...
# Import the data
data = pd.read_csv('/Users/nitinsinghal/Dropbox/DataScienceCourse/Data/kaggletortuga_techstudentcategory.csv')
...
testdata = pd.read_csv('/Users/nitinsinghal/Dropbox/DataScienceCourse/Data/kaggletortuga_techstudentcategory_tobescored.csv')
print(data.head())
data.fillna(0, inplace=True)
data.drop_duplicates(inplace=True)
X_test = data.iloc[:, 3:-1].values
y_testpred = clustermodel.fit_predict(X)
...
# Accuracy metrics
print(metrics.silhouette_score(X_test, clustermodel.labels_, metric='euclidean'))
